...
def plot_gmm_ordered_proximity_matrix(X_final_preprocessed, gmm_labels, sample_size=5000):
    """
    Computes Proximity and Incidence matrices for GMM, calculates their Pearson 
    correlation coefficient, and renders the high-definition Ordered Proximity Matrix Heatmap.
    """
    print(f"\n--- Computing GMM Ordered Proximity Matrix & Correlation (N={sample_size}) ---")
    ...
    if X_final_preprocessed.shape[0] > sample_size:
        ...
        labels_sample = np.asarray(gmm_labels[indices], dtype=int)
    else:
        X_sample = X_final_preprocessed.astype(np.float32)
        labels_sample = np.asarray(gmm_labels, dtype=int)
    ...
    gmm_inc_prox_correlation = np.corrcoef(flat_proximity, flat_incidence)[0, 1]
    print(f"Matrix Correlation (Proximity vs Incidence) for GMM (N={sample_size}): {gmm_inc_prox_correlation:.4f}")
    ...
    plt.title(f"Ordered Proximity Matrix | Gaussian Mixture Model (K=4) | N={sample_size} | Correlation: {gmm_inc_prox_correlation:.4f}", fontsize=12, fontweight='bold', pad=15)
    plt.tight_layout()
    plt.show()
    
    return gmm_inc_prox_correlation