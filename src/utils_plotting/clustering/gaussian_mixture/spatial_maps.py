...
def plot_definitive_gmm_map(df_coords_clean, gmm_labels):
    """
    Renders the high-contrast definitive geographical map mapping the 4 
    probabilistic GMM management zones across the Capitanata coordinate grid space.
    """
    print("\n--- Generating Definitive GMM Geographical Cluster Map ---")
    
    # 1. Create the synchronized spatial dataframe linking coordinates and K=4 GMM labels
    df_map = df_coords_clean.copy()
    df_map['Soil_Cluster'] = gmm_labels
    ...
    # 3. Render the definitive probabilistic geographical map
    ...
    sns.scatterplot(
        ...
        hue='Soil_Cluster',
        data=df_map,
        palette='Set1',       # High-contrast categorical palette for the 4 GMM zones
        alpha=0.9,
        s=1,                  # Minimal marker size to ensure ultra-high definition and prevent pixel bleeding
        edgecolor=None
    )

    plt.title("Capitanata Precision Agriculture: Definitive 4-Zone Gaussian Mixture Model Map", fontsize=14, fontweight='bold', pad=15)
    plt.xlabel("Spatial Coordinate X / Longitude")
    plt.ylabel("Spatial Coordinate Y / Latitude")
    plt.grid(True, linestyle=':', alpha=0.4)
    plt.legend(title="Management Zone (GMM Cluster)", loc="upper right", markerscale=10)
    plt.axis('equal')       # Enforces a strict 1:1 aspect ratio to completely eliminate geographic distortion
    plt.tight_layout()
    plt.show()


def plot_gmm_satellite_map(df_coords_clean, gmm_labels, output_filename='Capitanata_Satellite_GMM_Map.png'):
    """
    Overlays translucent probabilistic GMM fields over real-time ESRI high-resolution 
    satellite imagery via contextily to evaluate morphological and geographical ground-truth.
    """
    print("\n--- Generating Translucent GMM Satellite Ground-Truth Validation Map ---")
    ...
    ax = sns.scatterplot(
        ...
        edgecolor=None,
        zorder=2               # Ensures the probabilistic clustering layer is rendered on top
    )
    ...
    plt.title("Capitanata Precision Agriculture: Translucent Satellite Ground-Truth Validation (4-Zone Gaussian Mixture Model)", fontsize=14, fontweight='bold', pad=15)
    ...
    plt.legend(title="GMM Management Zone", loc="upper right", markerscale=8, facecolor='white', framealpha=0.9)
    plt.axis('equal')
    plt.tight_layout()

    # Save the final high-resolution, readable chart for the thesis attachments
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    print(f"GMM satellite validation map saved successfully as: {output_filename}")
    plt.show()