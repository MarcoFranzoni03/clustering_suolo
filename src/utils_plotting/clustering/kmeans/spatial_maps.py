...
def plot_definitive_cluster_map(df_coords_clean, final_labels):
    """
    Renders the high-contrast definitive geographical map mapping the 4 
    agronomic management zones across the Capitanata coordinate grid space.
    """
    print("\n--- Generating Definitive Geographical Cluster Map ---")
    
    # 1. Create the synchronized spatial dataframe linking coordinates and K=4 labels
    ...
    # 3. Render the definitive geographical map
    ...
    sns.scatterplot(
        ...
        hue='Soil_Cluster',
        data=df_map,
        palette='Set1',       # Clear, high-contrast colors for the 4 zones
        alpha=0.9,
        s=1,                  # Smallest marker size to prevent pixel bleeding and overlap
        edgecolor=None
    )

    plt.title("Capitanata Precision Agriculture: Definitive 4-Zone Management Map", fontsize=14, fontweight='bold', pad=15)
    plt.xlabel("Spatial Coordinate X / Longitude")
    plt.ylabel("Spatial Coordinate Y / Latitude")
    plt.grid(True, linestyle=':', alpha=0.4)
    plt.legend(title="Management Zone (Cluster)", loc="upper right", markerscale=10)
    plt.axis('equal')       # Strictly prevents structural or geographic distortion
    plt.tight_layout()
    plt.show()


def plot_satellite_ground_truth_map(df_coords_clean, final_labels, output_filename='Capitanata_Satellite_KMeans_Map.png'):
    """
    Overlays translucent clustering fields over real-time ESRI high-resolution 
    satellite imagery via contextily to evaluate morphological and geographical ground-truth.
    """
    print("\n--- Generating Translucent Satellite Ground-Truth Validation Map ---")
    ...
    # 2. Initialize the geographical plot figure
    ...
    ax = sns.scatterplot(
        ...
        hue='Soil_Cluster',
        data=df_map,
        palette='Set1',        # High-contrast color palette for clear zoning boundaries
        alpha=0.35,            # Perfectly balanced transparency to see both clusters and terrain
        s=1.0,                 # Increased size to ensure visibility over high-res satellite textures
        edgecolor=None,
        zorder=2               # Ensures clustering layer is rendered on top
    )
    ...
    try:
        cx.add_basemap(
            ...
            zorder=1                                  # Forces the background layer underneath
        )
    ...
    plt.title("Capitanata Precision Agriculture: Translucent Satellite Ground-Truth Validation (4-Zone Layout)", fontsize=14, fontweight='bold', pad=15)
    ...
    plt.legend(title="Agronomic Management Zone", loc="upper right", markerscale=8, facecolor='white', framealpha=0.9)
    plt.axis('equal')
    plt.tight_layout()

    # Save the final high-resolution, readable chart for the thesis attachments
    plt.savefig(output_filename, dpi=300, bbox_inches='tight')
    print(f"High-resolution satellite validation map saved successfully as: {output_filename}")
    plt.show()